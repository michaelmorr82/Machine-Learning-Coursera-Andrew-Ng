...
for i = 1:1:grad_desc_i
    ...
    dJ_theta_0 = (1/m) * sum(theta(1) .* x_data(:,1) + theta(2) .* x_data(:,2) - y_data(:));
    dJ_theta_1 = (1/m) * sum((theta(1) .* x_data(:,1) + theta(2) .* x_data(:,2) - y_data(:)).*x_data(:,2));
...
%                         create video of fit
            %=========================================================
    y_data_fit_temp = x_data * theta'; 
    
     if create_video == 1 
         i
         iteration_txt_vid = ['iteration = ' num2str(i)]
         alpha_txt_vid = ['\alpha = ' num2str(alpha)];
         theta0_txt_vid = ['\theta_0 = ' num2str(theta(1))];
         theta1_txt_vid = ['\theta_1 = ' num2str(theta(2))];
         J_txt_vid = ['J(\theta_0, \theta_1) = ' num2str(J_grad_desc(i))];
            ...
            axes1 = axes(...
              ...
              plot(x_data(:,2), y_data_fit_temp, '-');
              box('on');
              grid('on');
            title('Plotting Raw Data with Iteration Fit', 'FontSize', 25);
            xlabel('X - Data','FontSize',20)
            ylabel('Y - Data','FontSize',20)
            ylim([-5 25])
            xlim([0 25])
         ...
         'FontWeight','bold',...
         'Fontsize', 18,...
         'String',{iteration_txt_vid, alpha_txt_vid, theta0_txt_vid, theta1_txt_vid, J_txt_vid});
           % pause(1)
            A(i) = getframe(figure(2));
            delete(figure(2));
     end
...
fprintf('\n\nCreating Video... ')
movie2avi(A,'GradDescent.avi','fps',15,'quality',100)
...
%                         3-D Data
%=========================================================================
theta0_vals = -10:1:100;
theta1_vals = -10:1:100;
...
%=========================================================================
 ...
 figure('Color',[1 1 1],...
    ...
    axes1 = axes(...
        'Color',[0.99 0.99 0.99],...
        'FontSize',20); 
      plot(theta_guess(:,1),'r', 'LineWidth', 4);hold on;
      plot(theta_guess(:,2), 'b','LineWidth', 4);
      box('on');
      grid('on');
    title('theta''s', 'FontSize', 25);
    xlabel('Gradient Descent Iteration','FontSize',20)
    ylabel('Values of theta','FontSize',20)
