...
for iter = 1:1:Grad_iter
    ...
    hyp = theta(iter,1) * X_norm(:,1) + theta(iter,2) * X_norm(:,2) + theta(iter,3) * X_norm(:,3);
...
fprintf('Final/original Cost Function: %.2f %%\n',(J_final*100)/J(1));
%Testing results

Price_calc = theta_final(1) + theta_final(2) * X(:,1) + theta_final(3) * X(:,2) ;

Price_Err = (Price_calc - y)*100./y;

fprintf('Average error = %.3f %%',mean(Price_Err));
